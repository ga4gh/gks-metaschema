...
def test_prepare_release_warns_for_dirty_product_repo_by_default(tmp_path: Path) -> None:
    ...
    assert "Warning: Product gks-core has uncommitted changes. Review the final diff carefully." in messages
...
def test_prepare_release_warns_for_dirty_submodule_by_default(tmp_path: Path) -> None:
    ...
    assert "Warning: Submodule vrs has uncommitted changes. Review the final diff carefully." in messages
...
def test_prepare_release_skips_branch_warning_for_product_without_submodule(tmp_path: Path) -> None:
    ...
    assert not any(message.startswith("Warning: product branch") for message in messages)
...
def test_prepare_release_initializes_missing_submodule(tmp_path: Path) -> None:
    ...
    def runner(command: list[str], cwd: Path) -> None:
        ...
        commands.append((command, cwd))
        if command == ["git", "submodule", "update", "--remote", "--init", "--", "schema/submodules/vrs"]:
